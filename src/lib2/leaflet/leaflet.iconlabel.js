L.Icon.Label = L.Icon.extend({
	...
	_createLabel: function (img) {
		...
		// set up label
		label.className = 'leaflet-marker-iconlabel leaflet-marker-iconlabel-right ' + this.options.labelClassName;
		...
		if(this.options.classStyle){
			Object.keys(this.options.classStyle).forEach((function(key){
				label.style[key] = this.options.classStyle[key];
			}).bind(this));
		}
